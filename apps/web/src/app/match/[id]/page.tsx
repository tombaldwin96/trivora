...
export default function MatchPage() {
  ...
  const [profiles, setProfiles] = useState<Record<string, { username: string; display_name?: string }>>({});
  ...
  useEffect(() => {
    ...
    supabase.from('profiles').select('id, username, display_name').in('id', [match.player_a, match.player_b]).then(({ data }) => {
      const map: Record<string, { username: string; display_name?: string }> = {};
      ((data ?? []) as { id: string; username: string; display_name?: string }[]).forEach((p) => { map[p.id] = p; });
      setProfiles(map);
    });
    ...
    <div className="max-w-lg mx-auto py-6">
      ...
      <Card className="p-6">
        <div className="flex justify-between items-center">
          <div>
            <p className="font-medium">{profiles[match.player_a]?.display_name || profiles[match.player_a]?.username || 'Player A'}</p>
            <p className="text-2xl font-bold text-brand-600">{match.points_a}</p>
          </div>
          <span className="text-slate-400">vs</span>
          <div className="text-right">
            <p className="font-medium">{profiles[match.player_b]?.display_name || profiles[match.player_b]?.username || 'Player B'}</p>
