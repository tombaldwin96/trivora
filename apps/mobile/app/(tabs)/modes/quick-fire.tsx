...
async function playCoinCelebration() {
  try {
    ...
    sound.setOnPlaybackStatusUpdate((status) => {
      if (status.isLoaded && status.didJustFinishAndNotStop) {
        sound.unloadAsync();
      }
    });
  } catch {
    // ignore sound errors (e.g. no asset, network)
  }
  try {
...
const RECENT_IDS_KEY = 'trivora_quickfire_recent_ids';
...
async function getRecentQuestionIds(): Promise<Set<string>> {
  try {
    const raw = await SecureStore.getItemAsync(RECENT_IDS_KEY);
...
async function addRecentQuestionIds(ids: string[]): Promise<void> {
  try {
    const raw = await SecureStore.getItemAsync(RECENT_IDS_KEY);
    ...
    await SecureStore.setItemAsync(RECENT_IDS_KEY, JSON.stringify(trimmed));
  } catch {
    // ignore
  }
}
...
function pickTenWithLoadBalance(pool: Question[], recentIds: Set<string>): Question[] {
  ...
  const shuffledNew = shuffle(notRecent);
  const shuffledRecent = shuffle(recent);
  const combined = [...shuffledNew, ...shuffledRecent];
  return combined.slice(0, QUICK_FIRE_COUNT);
}
...
const COIN_COUNT_MAX = 12; // cap for performance
...
export default function QuickFireScreen() {
  ...
  const coinsDoneRef = useRef(false);
  ...
  const loadQuestions = useCallback(async () => {
    setLoading(true);
    const recentIds = await getRecentQuestionIds();
    ...
    setQuestions(final);
    addRecentQuestionIds(final.map((q) => q.id));
    setLoading(false);
  }, []);
  ...
  useEffect(() => {
    ...
    supabase.rpc('increment_profile_stats', {
      p_quizzes_delta: 1,
      p_correct_delta: score,
      p_incorrect_delta: incorrect,
    }).then(() => {}, () => {});
  ...
  // Countdown: 60 seconds total for all 10 questions (starts only after user taps Start)
  ...
  }, [gameStarted, questions.length, timesUp, gameComplete, index]);
  ...
  if (gameEnded) {
    ...
    return (
      <LinearGradient colors={['#1e1b4b', '#312e81']} style={styles.resultWrap}>
        {/* Level badge at top — coins fly here */}
        ...
        <View style={styles.resultCard}>
          ...
          <Text style={styles.resultTitle}>Quick Fire 10</Text>
          {timesUp && (
            <Text style={styles.resultTimesUp}>Time's up!</Text>
          )}
          ...
          {score === QUICK_FIRE_COUNT && (
            <Pressable
              ...
              onPress={() => {
                Share.share({
                  message: `I scored 10/10 on Quick Fire 10 in Trivora! Can you beat my score?`,
                  title: 'Quick Fire 10 - Perfect score!',
                }).catch(() => {});
              }}
            >
              <Ionicons name="share-social" size={22} color="#fbbf24" />
              <Text style={styles.shareTileText}>
                You scored 10/10! Share this result with your friends.
              </Text>
            </Pressable>
          )}
          <Pressable
    ...
    <View style={styles.container}>
      {!gameStarted && (
        <View style={styles.startOverlay}>
          ...
          <Pressable style={[styles.headerBackBtn, { top: insets.top }]} onPress={() => router.back()}>
            <Ionicons name="chevron-back" size={24} color="#fff" />
            <Text style={styles.headerBackText}>Back</Text>
          </Pressable>
          <View style={styles.startContent}>
            <Text style={styles.startTitle}>Quick Fire 10</Text>
      ...
      <View style={[styles.topRow, { paddingTop: insets.top }]}>
        <Pressable style={styles.headerBackBtnInline} onPress={() => router.back()}>
          <Ionicons name="chevron-back" size={22} color="#94a3b8" />
          <Text style={styles.headerBackTextInline}>Back</Text>
        </Pressable>
...
const styles = StyleSheet.create({
  ...
  headerBackText: { color: '#fff', fontSize: 17, fontWeight: '600' },
  headerBackBtnInline: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 4,
  ...
  headerBackTextInline: { color: '#94a3b8', fontSize: 16, fontWeight: '600' },
  topRow: { flexDirection: 'row', alignItems: 'center', gap: 16, marginBottom: 20 },
  progressRow: { flex: 1, marginBottom: 0, minWidth: 0 },
