-- Ignore stale quick-match sessions: only return or use matches that are recent.
-- 1) When returning "already in a match", only return if that match was created in the last 15 minutes.
--    Otherwise the user fell out of an old session (or never finished) and should get a fresh one.
-- 2) Keeps existing "online only" claim filter (creator last_seen_at and match created_at window).
...
BEGIN
  ...
  -- 2. Already in a *recent* pending/in_progress match? Return it. Ignore sessions older than 15 minutes.
  ...
  -- 3. Try to claim oldest waiting match where creator is online and match is recent.
  EXECUTE format(
    $q$
...
COMMENT ON FUNCTION public.quick_match_enter(UUID) IS
  'Quick match: return only recent existing match (created < 15 min ago), or claim a waiting match if creator is online, or create new. Ignores stale sessions.';
