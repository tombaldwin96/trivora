-- Add country to profile card (show in leaderboard modal only when set by user).
...
COMMENT ON FUNCTION public.get_user_profile_card(UUID) IS 'Public profile card for leaderboard: username, level, xp, quizzes, wins, losses, correct/incorrect, joined_at, country (when set).';
