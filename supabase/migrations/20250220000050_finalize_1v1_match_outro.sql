-- Set outro_started_at when match is finalized so both clients can show synced outro cutscene.
...
DECLARE
  ...
  v_rounds_total INT;
  v_winner_id UUID;
  v_outro_started_at TIMESTAMPTZ := NOW();
  v_outro_duration_ms INT := 2500;
BEGIN
  ...
  SET
    status = 'completed',
    ended_at = v_outro_started_at,
    result = jsonb_build_object(
      'winner_id', v_winner_id,
      'score_a', v_match.points_a,
      'score_b', v_match.points_b
    ),
    outro_started_at = v_outro_started_at,
    outro_duration_ms = v_outro_duration_ms,
    updated_at = NOW()
  WHERE id = p_match_id;
...
COMMENT ON FUNCTION public.finalize_1v1_match(UUID) IS
  'Marks match completed, sets outro_started_at for cinematic, and updates standings when both players have answered all rounds.';
