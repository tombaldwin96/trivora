...
/** Update leaderboard snapshot for a session (top 25). Call from admin dashboard ticker or after reveal. Body: { session_id } */
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: scoresRaw } = await supabase
      ...
      .select('user_id, total_score, correct_count, answered_count')
    ...
    const { data: profiles } = await supabase
      .from('profiles')
      .select('id, username, avatar_url, country, level, live_quiz_win_count')
      .in('id', topScores.map((r: { user_id: string }) => r.user_id));

    const profileMap = Object.fromEntries(((profiles ?? []) as { id: string; username: string; avatar_url: string | null; country: string | null; level: number; live_quiz_win_count: number }[]).map((p) => [p.id, p]));
    const topJson = topScores.map((r: { user_id: string; total_score: number; correct_count: number; answered_count: number }, i: number) => {
      ...
      return {
        ...
        level: prof?.level ?? 1,
        live_quiz_win_count: prof?.live_quiz_win_count ?? 0,
    ...
    await supabase.from('live_quiz_leaderboard_snapshot').upsert(
      { session_id: sessionId, top_json: topJson, updated_at: now },
      { onConflict: 'session_id' }
    );

    return new Response(
      JSON.stringify({ ok: true, updated_at: now, count: topJson.length }),
