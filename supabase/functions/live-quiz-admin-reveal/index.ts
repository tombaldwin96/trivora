...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { error: stateErr } = await supabase.from('live_quiz_state').update({ phase: 'reveal', reveal_started_at: now, updated_at: now }).eq('session_id', sessionId);
    ...
    const { data: profiles } = await supabase.from('profiles').select('id, username, avatar_url, country').in('id', topScores.map((r: { user_id: string }) => r.user_id));
    const profileMap = Object.fromEntries(((profiles ?? []) as { id: string; username: string; avatar_url: string | null; country: string | null }[]).map((p) => [p.id, p]));
    const topJson = topScores.map((r: { user_id: string; total_score: number; correct_count: number; answered_count: number }, i: number) => ({ rank: i + 1, user_id: r.user_id, username: profileMap[r.user_id]?.username ?? null, avatar_url: profileMap[r.user_id]?.avatar_url ?? null, country: profileMap[r.user_id]?.country ?? null, total_score: r.total_score, correct_count: r.correct_count, answered_count: r.answered_count }));
    ...
    await supabase.from('live_quiz_admin_actions').insert({ session_id: sessionId, admin_user_id: user.id, action_type: 'REVEAL', payload: { reveal_started_at: now } });
    return new Response(JSON.stringify({ ok: true, phase: 'reveal', reveal_started_at: now, leaderboard_updated: true }), { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
  } catch (e) {
