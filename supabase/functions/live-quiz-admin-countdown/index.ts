...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const minutes = Math.max(0, Math.min(120, Number(body.minutes) ?? 5));
    ...
    const { error: stateErr } = await supabase.from('live_quiz_state').update({ phase: 'countdown', countdown_ends_at: endsAt, updated_at: new Date().toISOString() }).eq('session_id', sessionId);
    ...
    await supabase.from('live_quiz_sessions').update({ status: 'scheduled', updated_at: new Date().toISOString() }).eq('id', sessionId);
    await supabase.from('live_quiz_admin_actions').insert({ session_id: sessionId, admin_user_id: user.id, action_type: 'COUNTDOWN', payload: { minutes, countdown_ends_at: endsAt } });
    return new Response(JSON.stringify({ ok: true, phase: 'countdown', countdown_ends_at: endsAt }), { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
  } catch (e) {
